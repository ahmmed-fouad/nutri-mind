  ...
  return (
    <section className="min-h-[80vh] flex flex-col items-center justify-center bg-gradient-to-br from-white via-blue-50 to-green-50 py-10 px-2">
      <div className="max-w-3xl w-full glassmorphism p-8 rounded-3xl shadow-2xl mb-8 animate-fade-in">
        <h1 className="text-3xl md:text-4xl font-bold text-center mb-2 text-primary">Advanced Diet Calculator</h1>
        <p className="text-center text-zinc-500 mb-6">Personalize your nutrition plan with science-backed calculations and instant visual feedback.</p>
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleCalculate}>
          <div className="flex flex-col gap-3">
            <label className="font-semibold">Age</label>
            <input type="number" name="age" min={10} max={100} value={form.age} onChange={handleChange} className="input glassmorphism" required />
            <label className="font-semibold">Gender</label>
            <select name="gender" value={form.gender} onChange={handleChange} className="input glassmorphism">
              <option value="female">Female</option>
              <option value="male">Male</option>
            </select>
            <label className="font-semibold">Height (cm)</label>
            <input type="number" name="height" min={100} max={250} value={form.height} onChange={handleChange} className="input glassmorphism" required />
            <label className="font-semibold">Weight (kg)</label>
            <input type="number" name="weight" min={30} max={250} value={form.weight} onChange={handleChange} className="input glassmorphism" required />
            <label className="font-semibold">Activity Level</label>
            <select name="activity" value={form.activity} onChange={handleChange} className="input glassmorphism">
              {activityLevels.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}
            </select>
            <label className="font-semibold">Goal</label>
            <select name="goal" value={form.goal} onChange={handleChange} className="input glassmorphism">
              {goals.map(g => <option key={g.value} value={g.value}>{g.label}</option>)}
            </select>
            <label className="font-semibold">Dietary Preference</label>
            <select name="pref" value={form.pref} onChange={handleChange} className="input glassmorphism">
              {dietaryPrefs.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-3">
            <label className="font-semibold">Body Fat % (optional)</label>
            <input type="number" name="bodyFat" min={5} max={60} value={form.bodyFat} onChange={handleChange} className="input glassmorphism" />
            <label className="font-semibold">Waist (cm, optional)</label>
            <input type="number" name="waist" min={40} max={200} value={form.waist} onChange={handleChange} className="input glassmorphism" />
            <label className="font-semibold">Hip (cm, optional)</label>
            <input type="number" name="hip" min={40} max={200} value={form.hip} onChange={handleChange} className="input glassmorphism" />
            <label className="font-semibold">Meals per day</label>
            <input type="number" name="meals" min={1} max={8} value={form.meals} onChange={handleChange} className="input glassmorphism" />
            <label className="font-semibold">Allergies (comma separated)</label>
            <input type="text" name="allergies" value={form.allergies} onChange={handleChange} className="input glassmorphism" placeholder="e.g. peanuts, gluten" />
            <button type="submit" className="mt-6 btn btn-primary py-3 rounded-xl font-bold text-lg shadow-lg bg-primary text-white hover:bg-primary/90 transition-all">Calculate</button>
          </div>
        </form>
      </div>
      {showResults && results && (
        <div className="w-full max-w-4xl grid md:grid-cols-2 gap-8 animate-fade-in-up">
          <div className="glassmorphism p-6 rounded-2xl shadow-xl flex flex-col gap-4">
            <h2 className="text-2xl font-bold text-primary mb-2">Your Results</h2>
            <div className="flex flex-col gap-2 text-lg">
              <span><b>Calories:</b> {results.calories} kcal</span>
              <span><b>Protein:</b> {results.protein} g</span>
              <span><b>Carbs:</b> {results.carbs} g</span>
              <span><b>Fat:</b> {results.fat} g</span>
              <span><b>Water:</b> {results.water} L/day</span>
            </div>
          ...
          <div className="glassmorphism p-6 rounded-2xl shadow-xl flex flex-col gap-6">
            ...
            <ResponsiveContainer width="100%" height={120}>
              <BarChart data={calorieBarData}>
                ...
                <YAxis hide />
                <Tooltip />
                <Bar dataKey="value" fill={calorieBarColor} radius={[10, 10, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {showResults && (
        <div className="w-full max-w-4xl grid md:grid-cols-2 gap-8 mt-8 animate-fade-in-up">
          <div className="glassmorphism p-6 rounded-2xl shadow-xl flex flex-col gap-4">
            <h2 className="text-xl font-bold text-primary mb-2">Sample Meal Plan</h2>
            <ul className="flex flex-col gap-2">
              {mealPlan.map((m, i) => (
                <li key={i} className="flex items-center gap-3 text-lg">
                  <span className="text-2xl">{m.icon}</span>
                  <span><b>{m.meal}:</b> {m.desc}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="glassmorphism p-6 rounded-2xl shadow-xl flex flex-col gap-4">
            <h2 className="text-xl font-bold text-primary mb-2">Tips for Success</h2>
