          ...
          {plans.map((plan) => (
            ...
            >
              ...
              <div className="text-zinc-400 mb-4">
                {plan.price === 0 ? t("forever") : t("per_month")}
              </div>
              ...
              >
                {plan.price === 0 ? t("get_started") : t("upgrade")}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-4 sm:p-8 mb-12 md:mb-16">
          ...
          <div className="w-full h-64 sm:h-96">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={chartData.map(row => ({...row, feature: t(`chart_features.${row.feature}`)}))}>
                <PolarGrid />
          ...
          <ul className="space-y-6">
            {faqs.map((faq, i) => (
              <li key={i}>
                ...
                <div className="text-zinc-500 dark:text-zinc-400 text-sm sm:text-base">{t(`faqs.${i}.a`)}</div>
              </li>
            ))}
          </ul>
        </div>
