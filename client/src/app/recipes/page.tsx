  ...
  return (
    <div className="min-h-[90vh] bg-zinc-50 dark:bg-zinc-900/60 py-10 px-4">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold mb-8 text-center text-primary">Healthy Recipes</h1>
        <p className="text-center text-zinc-500 mb-12 max-w-2xl mx-auto text-lg">
          Discover delicious, healthy recipes with nutrition breakdowns. Click any recipe for details and a nutrition chart!
        </p>
        {isLoading && <div className="text-center text-lg">Loading recipes...</div>}
        {error && <div className="text-center text-red-500">Failed to load recipes.</div>}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {data?.meals?.map((recipe: any) => (
            ...
            >
              ...
              />
              <h2 className="text-xl font-semibold mb-2 text-center text-primary group-hover:underline">{recipe.strMeal}</h2>
              <div className="text-zinc-500 text-sm mb-2 text-center line-clamp-2">
                {recipe.strCategory} | {recipe.strArea}
              </div>
              <div className="flex gap-2 flex-wrap justify-center mb-2">
                {recipe.strTags?.split(",")?.slice(0, 3).map((tag: string) => (
                  <span key={tag} className="bg-primary/10 text-primary px-2 py-1 rounded text-xs font-medium">
                    {tag}
                  </span>
                ))}
              </div>
              <div className="text-zinc-400 text-xs">Calories: {Math.floor(Math.random() * 200) + 200} kcal</div>
            </div>
          ))}
        </div>
      </div>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1 flex flex-col gap-2">
                <h2 className="text-2xl font-bold text-primary mb-2">{selected.strMeal}</h2>
                <div className="text-zinc-500 text-sm mb-2">{selected.strCategory} | {selected.strArea}</div>
                <div className="mb-2">
                  ...
                  <ul className="list-disc ml-6 text-zinc-700 dark:text-zinc-200">
                    {Array.from({ length: 20 }).map((_, i) => {
                      ...
                      return ingredient && ingredient.trim() ? (
                        <li key={i}>{ingredient} {measure && <span className="text-zinc-400">({measure})</span>}</li>
                      ) : null;
                    })}
                  </ul>
                </div>
                <div className="mb-2">
                  ...
                  {/* <p className="text-zinc-600 dark:text-zinc-300 whitespace-pre-line mt-1">{selected.strInstructions}</p> */}
                </div>
                <div className="mb-2">
