    ...
    <div className="min-h-[95vh] bg-gradient-to-br from-zinc-50 via-white to-blue-50 dark:from-zinc-900 dark:via-zinc-950 dark:to-blue-950 py-10 px-2 md:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-2 text-primary">Admin Dashboard</h1>
          <p className="text-lg text-zinc-500 max-w-2xl mx-auto">Monitor platform activity, manage users, and view analytics.</p>
        </div>
        {/* Stat Cards */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-10">
          {statCards.map((card, i) => (
            <div key={i} className="bg-white dark:bg-zinc-900 rounded-xl shadow border border-zinc-200 dark:border-zinc-800 flex flex-col items-center py-5 px-2">
              <div>{card.icon}</div>
              <div className="text-2xl font-bold mt-2">{card.value}</div>
              <div className="text-xs text-zinc-400 mt-1">{card.label}</div>
              <div className={`flex items-center gap-1 text-xs mt-1 ${card.up ? 'text-green-500' : 'text-red-500'}`}>
                {card.up ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownRight className="w-4 h-4" />} {card.trend}%
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-8 mb-12">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <h2 className="text-2xl font-bold text-primary">Platform Analytics</h2>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-8 mb-12">
          <h2 className="text-xl font-bold text-primary mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5" /> Recent Activity</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-6">
            <h3 className="text-xl font-bold text-primary mb-4 flex items-center gap-2"><Users className="w-5 h-5" /> User Management</h3>
              ...
              <tbody>
                {demoUsers.map((u, i) => (
                  <tr key={i} className="border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <td className="py-2">
                      {u.status === "active" ? <span className="text-green-500 font-semibold">Active</span> : <span className="text-red-500 font-semibold">Banned</span>}
                    </td>
                    <td className="py-2 flex gap-2">
                      <button className="p-1 rounded hover:bg-accent"><Edit className="w-4 h-4" /></button>
                      {u.status === "active" ? (
                        <button className="p-1 rounded hover:bg-accent"><Ban className="w-4 h-4 text-red-500" /></button>
                      ) : (
                        <button className="p-1 rounded hover:bg-accent"><CheckCircle className="w-4 h-4 text-green-500" /></button>
                      )}
                      <button className="p-1 rounded hover:bg-accent"><Trash2 className="w-4 h-4 text-red-500" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="flex flex-col gap-6">
            {contentCards.map((c, i) => (
              <div key={i} className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-6 flex items-center gap-4">
                {c.icon}
                <div>
                  <div className="text-lg font-bold text-primary">{c.label}</div>
                  <div className="text-2xl font-bold mt-1">{c.count}</div>
                </div>
                <button className="ml-auto px-4 py-2 rounded-lg bg-primary text-white font-semibold shadow hover:bg-primary/90 transition">Manage</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {/* Support Overview */}
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-6">
            <h3 className="text-xl font-bold text-primary mb-4 flex items-center gap-2"><LifeBuoy className="w-5 h-5" /> Support Overview</h3>
            <div className="mb-4">
              <div className="flex gap-4 text-sm">
                <span className="text-green-500 font-semibold">Open: {supportTickets.filter(t => t.status === "open").length}</span>
                <span className="text-yellow-500 font-semibold">Pending: {supportTickets.filter(t => t.status === "pending").length}</span>
                <span className="text-zinc-400 font-semibold">Closed: {supportTickets.filter(t => t.status === "closed").length}</span>
              </div>
            </div>
            <ul className="divide-y divide-zinc-200 dark:divide-zinc-800">
              {supportTickets.map((t, i) => (
                <li key={i} className="py-2 flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-400 ml-auto">{t.user}</span>
                  <span className={`text-xs font-semibold ml-2 ${t.status === "open" ? "text-green-500" : t.status === "pending" ? "text-yellow-500" : "text-zinc-400"}`}>{t.status}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex flex-col items-center justify-center gap-6 bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-6">
            <h3 className="text-xl font-bold text-primary mb-4 flex items-center gap-2"><Download className="w-5 h-5" /> Export Data</h3>
            <button className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-primary to-blue-400 text-white font-bold shadow-lg hover:from-primary/90 hover:to-blue-500 transition text-lg"><Download className="w-6 h-6" /> Export Analytics</button>
          </div>
          {/* Admin Notes */}
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 p-6 flex flex-col h-full">
            <h3 className="text-xl font-bold text-primary mb-4 flex items-center gap-2"><NotebookPen className="w-5 h-5" /> Admin Notes</h3>
            <textarea
            ...
            />
            <button className="w-full flex items-center justify-center gap-2 py-2 rounded-lg bg-primary text-white font-semibold shadow hover:bg-primary/90 transition"><NotebookPen className="w-5 h-5" /> Save Note</button>
          </div>
        </div>
      </div>
    </div>
